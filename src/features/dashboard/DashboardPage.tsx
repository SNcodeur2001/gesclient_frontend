...
import {
  Users, ShoppingCart, Recycle, CreditCard,
  TrendingUp, TrendingDown, Leaf, ShoppingBag,
} from 'lucide-react'
...
function useRecentActivity() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const list = (data?.data?.items ?? data?.data ?? []) as NotificationResponseDto[]
...
function VariationBadge({ value }: { value: string }) {
  ...
  return (
    <p className={`text-xs font-bold mt-2 flex items-center gap-1 ${isDown ? 'text-red-500' : 'text-green-600'}`}>
  ...
  return (
    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex justify-between items-start">
      <div>
        ...
        <p className="text-2xl font-bold mt-1 text-slate-800 leading-tight">{value}</p>
...
function DashboardDirecteurPage() {
  ...
  const kpiCards = data
    ? [
        {
          label: 'Total Clients',
          value: formatNumber(data.clients.totalApporteurs + data.clients.totalAcheteurs),
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {isLoading
            ...
            : isError
            ? <p className="col-span-4 text-sm text-red-500">Impossible de charger les statistiques.</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  {caData[0]?.mois ?? '—'} – {caData[caData.length - 1]?.mois ?? '—'} 2026
                </p>
              </div>
            </div>
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height={192}>
                <AreaChart data={caData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                  <defs>
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h3 className="font-bold text-slate-800">Collectes de plastique (kg)</h3>
                <p className="text-xs text-slate-400 mt-0.5">
                  {collecteData[0]?.mois ?? '—'} – {collecteData[collecteData.length - 1]?.mois ?? '—'} 2026
                </p>
              </div>
            </div>
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height={192}>
                <BarChart data={collecteData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
            ...
            <a href="/notifications" className="text-[#2563EB] text-sm font-semibold hover:underline">
              Voir tout
            </a>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-6 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Nom</th>
                  <th className="px-6 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Tonnage (kg)</th>
                  <th className="px-6 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Total versé (FCFA)</th>
                </tr>
              </thead>
                ...
                {isLoading
                  ...
                  : (data?.topApporteurs ?? []).map((a) => (
                      <tr key={a.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-6 py-3 text-sm font-medium text-slate-800">{a.nom}</td>
                        <td className="px-6 py-3 text-sm text-slate-600">{formatNumber(a.tonnage)}</td>
                        <td className="px-6 py-3 text-sm font-bold text-slate-800">{formatNumber(a.montant)}</td>
                      </tr>
                    ))}
              </tbody>
            </table>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-6 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Nom</th>
                  <th className="px-6 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider">CA total (FCFA)</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {isLoading
                  ...
                  : (data?.topAcheteurs ?? []).map((a) => (
                      <tr key={a.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-6 py-3 text-sm font-medium text-slate-800">{a.nom}</td>
                        <td className="px-6 py-3 text-sm font-bold text-slate-800">{formatNumber(a.chiffreAffaires)}</td>
                      </tr>
                    ))}
              </tbody>
            </table>
...
const NOTIF_TYPE_STYLES: Record<string, { label: string; className: string }> = {
  NOUVELLE_COLLECTE:   { label: 'Collecte',  className: 'bg-teal-100 text-teal-700' },
  ACOMPTE_RECU:        { label: 'Paiement',  className: 'bg-green-100 text-green-700' },
  COMMANDE_PRETE:      { label: 'Commande',  className: 'bg-purple-100 text-purple-700' },
  COMMANDE_FINALISEE:  { label: 'Commande',  className: 'bg-green-100 text-green-700' },
  IMPORT_TERMINE:      { label: 'Import',    className: 'bg-indigo-100 text-indigo-700' },
  COMMANDE_EN_ATTENTE: { label: 'Commande',  className: 'bg-amber-100 text-amber-700' },
}
    ...
    <table className="w-full text-left border-collapse">
      <thead>
        <tr className="bg-slate-50">
          <th className="px-6 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Date/Heure</th>
      ...
      <tbody className="divide-y divide-slate-100">
        {logs.map((notif) => {
          const style = NOTIF_TYPE_STYLES[notif.type] ?? { label: notif.type, className: 'bg-slate-100 text-slate-500' }
          return (
            <tr key={notif.id} className="hover:bg-slate-50 transition-colors">
              ...
              <td className="px-6 py-4">
                <span className={`px-2 py-1 text-[11px] font-bold rounded uppercase ${style.className}`}>
                  {style.label}
                </span>
              </td>
